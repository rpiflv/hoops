...
app.get('/api/games/:year/:month/:day', async (req, res) => {
    try {
        const response = await axios.get(`https://api.sportradar.com/nba/trial/v8/en/games/${req.params.year}/${req.params.month}/${req.params.day}/schedule.json?api_key=${SPORTRADAR_KEY}`)
...
app.get('/api/games/:gameId', async (req, res) => {
    try {
        const response = await axios.get(`https://api.sportradar.com/nba/trial/v8/en/games/${req.params.gameId}/summary.json?api_key=${SPORTRADAR_KEY}`);
        // await response.json();
        res.send(response.data);
    } catch(err) {
        console.error(err);
...
app.get('/api/teams', async (req, res) => {
    try {
        const response = await axios.get(`http://api.sportradar.us/nba/trial/v8/en/seasons/2022/REG/standings.json?api_key=${SPORTRADAR_KEY}`);
        res.send(response.data);
    } catch(err) {
        console.error(err);
        try {
...
app.get('/api/teams/:teamId', async (req, res) => {
    try {
        const teamInfo = await axios.get(`http://api.sportradar.us/nba/trial/v8/en/teams/${req.params.teamId}/profile.json?api_key=${SPORTRADAR_KEY}`)
        const stats = await axios.get(`http://api.sportradar.us/nba/trial/v8/en/seasons/2022/REG/teams/${req.params.teamId}/statistics.json?api_key=${SPORTRADAR_KEY}`);
...
app.get('/api/:playerId', async (req, res) => {
    const playerId = req.params.playerId;
    try {
        const response = await axios.get(`http://api.sportradar.us/nba/trial/v8/en/players/${playerId}/profile.json?api_key=${SPORTRADAR_KEY}`);
        res.send(response.data);
    } catch(err) {
        console.error(err);
        try {
